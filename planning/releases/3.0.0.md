# modern-di-aiogram 3.0.0 — modern-di 3.x

Requires **modern-di >= 3, < 4**. **No public API change** — `FromDI`,
`aiogram_event_provider`, `aiogram_update_provider`, `fetch_di_container`,
`inject`, and `setup_di` keep their signatures and behavior. The adapter
already drove the explicit open/close container lifecycle — `async with`
around each per-update child container, and the root container opened /
closed via `dispatcher.startup.register(container.open)` /
`dispatcher.shutdown.register(container.close_async)` — so no adapter code
changed for
